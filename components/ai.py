...
class HostileEnemy(BaseAI):
    ...
    def perform(self) -> None:
        ...
        distance = max(abs(dx), abs(dy))  # Chebyshev distance (or chessboard distance)

        # Determine what the enemy can see
        entity_visibility_map = tcod.map.compute_fov(
            self.engine.game_map.tiles["transparent"],
            (self.entity.x, self.entity.y),
            radius=self.entity.eyesight_radius)

        # if self.engine.game_map.visible[self.entity.x, self.entity.y]:
        if entity_visibility_map[self.engine.player.x, self.engine.player.y]:  # The player is visible to the entity
            if distance <= 1:
